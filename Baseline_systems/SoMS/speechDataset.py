...
class speechDataset(Dataset):
    ...
    def __init__(self, audioPath, csvPath, filesExt=".csv", delim=';', classFunc=lambda y: y, sr=None, WinSize=0.025, WinShift=0.010, n_mels=128, shouldNormalize=True,
                 colomnNames="File", colomnsSeeked=["Valance", "Valance_0"], labeldtype="f", noTimeStamps=False, skip_header=True, skip_instname=True): # added , "Valence_0" into the colomnsSeeked array ******
    ...
    def create_paths_and_labels(self): 
        ...
        for i, fileName in enumerate(my_list):
            print("\n\npreparing files %d%%" % int(100*i/(filesCount)))
            ...
            for colomnSeeked in self.colomnsSeeked:
                ...
                if not math.isnan(label):
                       print("self.classFunc - ", self.classFunc(label))
                       label = self.classFunc(label)
                else:
                       print("\relse if NAN -", label)		 
                labels.append(label)
            self.labels.append(labels)
        print("\npreparing files completed %s \n" %self.csvPath)
