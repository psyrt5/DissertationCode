...
class ModelWrapper():
    def __init__(self, models:[nn.Module], loadFromPath=None, numModels=2, tabuList=[], learningRate=0.001, criterion="CCC-cov", shuffle=True, device='cpu'): # changed LR from 0.01
        self.models = models
    ...
    def train(self, dataset:Dataset, batchSize=1, epochs=10, firstEpoch=1, savePath="models/first", evalDataset:Dataset=None, csvPath="models/train.csv", 
              earlyStopAfter=10, tolerance=15, computeLossFor=10 ,computeLossForEval=10, tarsFunc=lambda tars: tars[:,0], plusTar=-1):#*****************EPOCHS CHANGED FROM 10 To 5 & TARS NOT CHANGED
    ...
    def testConfMatrix(self, dataset:Dataset, batchSize=1, verbose=True, numTargets=10, classFunc=lambda y: int(y)-1, tarsFunc=lambda tars: tars[:,0], plusTar=-1):# TARS NOT CHANGED
    ...
    def trainCompute(self, dataset:Dataset, batchSize=1, computeLossFor=20, tarsFunc=lambda tars: tars[:,0], plusTar=-1): # Tars not changed
    ...
    def testCompute(self, dataset:Dataset, batchSize=1, verbose=False, computeLossFor=20, tarsFunc=lambda tars: tars[:,0], plusTar=-1): # TARS NOT CHANGED
        test_loader = DataLoader(dataset=dataset, batch_size=batchSize, shuffle=self.shuffle)
        for i, model in enumerate(self.models):
            self.models[i].eval()
        return self.computeLossFor(test_loader, computeLossFor, tarsFunc=tarsFunc, plusTar=plusTar)

    def computeLossFor(self, data_loader, computeLossFor, tarsFunc=lambda tars: tars[:,0], plusTar=-1): # TARS NOT CHANGED
        correct = 0
        allLoss = 0
        counter = 0
        ...
        for batch_idx, (data, target) in enumerate(data_loader):
            ...
            print("Target-Prediction = ", (target).data.view_as(preds))
            print("Accumulative Correct = ", correct)
            ...
            if not self.models[0].training: 
                ...
                if plusTar >= 0:   
                    tar0 = tars[:,plusTar].to(device=self.device)#tars[:,plusTar]. ***********************************************************************CHANGED FROM THIS TO tars.to which allowed to test single val
                    output, target = output + tar0, target + tar0
                    
            print("Output, Target, Preds: ", output, target, preds)
            ...
            if ((batch_idx+1)*len(data) % computeLossFor == 0): #or ((batch_idx+1)*len(data)==len(data_loader.dataset)) :
                ...
                outputs, targets = torch.FloatTensor([]), torch.FloatTensor([])
                outputs, targets = outputs.to(device=self.device), targets.to(device=self.device)# Resetting outputs and targets

        accuracy = 100. * correct / len(data_loader.dataset)
        if counter!=0:
	        epochLoss = allLoss / counter #/ len(data_loader.dataset)
        else:
	        epochLoss = 999 #************************************************************************This was added incase counter is 0 -> if no correct predictions are made
        print("8")
        if isinstance(accuracy,int):
        	return accuracy, epochLoss
        if isinstance(epochLoss,int):
        	return accuracy, epochLoss
